...
export const RealtimePreviewBadge: React.FC<RealtimePreviewBadgeProps> = ({
  ...
  return (
    <div className="w-full rounded-2xl bg-[#121620]/90 p-4 border border-white/10 shadow-xl transition-all duration-300">
      <div className="flex items-center justify-between text-xs font-semibold text-zinc-400 mb-3 border-b border-white/5 pb-2.5">
        ...
        <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-mono flex items-center gap-1.5 transition-all ${
          isPairReady
            ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
            : 'bg-white/5 text-zinc-400 border border-white/10'
        }`}>
          <CheckCircle2 className="w-3 h-3" />
          {isPairReady ? 'Brand Matched' : 'Awaiting Domain Input'}
        </span>
      </div>

      <div className="flex items-center justify-between gap-4 py-2 px-3 rounded-xl bg-[#090a0d] border border-white/5">
        {/* Sender Logo Container */}
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#181d2a] border border-white/10 p-2 flex items-center justify-center relative">
            {senderLogo ? (
              <img
                src={senderLogo}
                alt={senderName}
          ...
          <div className="flex flex-col">
            <span className="text-[10px] uppercase font-mono tracking-widest text-zinc-500">Your site</span>
            <span className="text-sm font-bold text-white max-w-[120px] truncate">{senderName}</span>
          </div>
        </div>
        ...
        <div className="relative flex flex-col items-center justify-center">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
            ...
            <Zap className={`w-4 h-4 ${isPairReady ? 'fill-black' : 'fill-[#e2f952]/20'}`} />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex flex-col text-right">
            <span className="text-[10px] uppercase font-mono tracking-widest text-zinc-500">Their site</span>
            <span className="text-sm font-bold text-white max-w-[120px] truncate">{prospectName}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-[#181d2a] border border-white/10 p-2 flex items-center justify-center relative">
            {prospectLogo ? (
              <img
